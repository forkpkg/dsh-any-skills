...
const CSS = [
  ...
  '.dsh-as-btn2{display:inline-flex;align-items:center;justify-content:center;gap:6px;min-height:32px;padding:0 14px;border-radius:9px;border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.28));background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.08));color:var(--dsw-alias-label-primary,#e6ebf2);font-size:12.5px;font-weight:500;cursor:pointer;white-space:nowrap;transition:background .15s ease,color .15s ease,border-color .15s ease,box-shadow .15s ease,transform .1s ease,filter .15s ease}',
  '.dsh-as-btn2:hover:not(:disabled){border-color:var(--dsw-alias-brand-primary,#4f8cff);color:var(--dsw-alias-brand-primary,#4f8cff);background:rgba(79,140,255,.1)}',
  '.dsh-as-btn2:active:not(:disabled){transform:scale(.97)}',
  '.dsh-as-btn2:focus-visible{outline:2px solid var(--dsw-alias-brand-primary,#4f8cff);outline-offset:1px}',
  '.dsh-as-btn2:disabled{opacity:.45;cursor:not-allowed;box-shadow:none;transform:none}',
  '.dsh-as-btn2.dsh-as-primary{background:linear-gradient(180deg,#5b96ff 0%,var(--dsw-alias-brand-primary,#4f8cff) 45%,#3d78e6 100%);border-color:transparent;color:#fff;box-shadow:0 1px 2px rgba(0,0,0,.25),0 4px 10px rgba(79,140,255,.28)}',
  '.dsh-as-btn2.dsh-as-primary:hover:not(:disabled){color:#fff;filter:brightness(1.08);box-shadow:0 2px 4px rgba(0,0,0,.28),0 6px 14px rgba(79,140,255,.4);transform:translateY(-1px)}',
  '.dsh-as-btn2.dsh-as-primary:active:not(:disabled){transform:translateY(0) scale(.98);box-shadow:0 1px 2px rgba(0,0,0,.25)}',
  '.dsh-as-btn2.dsh-as-danger{color:var(--dsw-alias-danger,#e05c5c);border-color:rgba(224,92,92,.35);background:rgba(224,92,92,.06)}',
  '.dsh-as-btn2.dsh-as-danger:hover:not(:disabled){color:#e05c5c;border-color:#e05c5c;background:rgba(224,92,92,.12)}',
